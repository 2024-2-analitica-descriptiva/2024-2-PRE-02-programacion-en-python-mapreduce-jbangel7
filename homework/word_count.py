...
import fileinput
import glob # libreria para leer directorios y saber archivos tienen (Jomba)
import os.path
import string # libreria para borrar puntuaciones
...
def line_preprocessing(sequence):
    ...
    sequence = [
        (key, value.translate(str.maketrans("", "", string.punctuation))) # maketrans elimina toda la puntuacion
        for key, value in sequence
    ]    

    sequence = [
        (key, value.lower())  # convertir todo a minusculas
        for key, value in sequence
    ]

    sequence = [
        (key, value.strip())  #strip elimina los espacios vacios
        for key, value in sequence
    ]
    return sequence
